...
describe("ui.requestInput", () => {
  it("settles a blocking request through the harness", async () => {
    ...
    const pending = patcher.ui.requestInput({
  ...
  it("settles requests on abort and plugin disposal", async () => {
    ...
    const aborted = first.patcher.ui.requestInput(
      {
        threadId: "thread-test",
        rendererId: "form",
        title: "Form",
    ...
    const disposed = second.patcher.ui.requestInput({
      threadId: "thread-test",
      rendererId: "form",
      title: "Form",
      payload: null,
...
describe("storage", () => {
  it("kv round-trips JSON, lists by prefix sorted, and enforces the 256KB cap", async () => {
    const { patcher } = createFakePluginHost();
    await patcher.storage.kv.set("slack:b", { channel: "C1" });
    await patcher.storage.kv.set("slack:a", 42);
    await patcher.storage.kv.set("other", "x");
    expect(await patcher.storage.kv.get("slack:b")).toEqual({ channel: "C1" });
    expect(await patcher.storage.kv.list("slack:")).toEqual([
      "slack:a",
      "slack:b",
    ]);
    expect(await patcher.storage.kv.list()).toEqual([
      "other",
      "slack:a",
      "slack:b",
    ]);
    await patcher.storage.kv.delete("slack:a");
    expect(await patcher.storage.kv.get("slack:a")).toBeUndefined();

    await expect(
      patcher.storage.kv.set("big", "x".repeat(256 * 1024)),
  ...
  it("database() returns one shared database and migrate() is append-only by index", () => {
    ...
    const again = patcher.storage.database();
    expect(again).toBe(db);
    patcher.storage.migrate(again, [
      "CREATE TABLE notes (id INTEGER PRIMARY KEY, body TEXT)",
...
describe("settings", () => {
  function defineSettings(patcher: PatcherPluginApi) {
    return patcher.settings.define({
  ...
  it("resolves pre-seeded values, defaults, and type mismatches like the host", async () => {
    ...
    const handle = defineSettings(patcher);
  ...
  it("rejects duplicate and invalid descriptors at define time", () => {
    const { patcher } = createFakePluginHost();
    defineSettings(patcher);
    expect(() =>
      patcher.settings.define({ token: { type: "string", label: "Again" } }),
...
describe("rpc", () => {
  it("callRpc validates and JSON-normalizes input and output", async () => {
    const { patcher, harness } = createFakePluginHost({ pluginId: "notes" });
  ...
  it("rejects invalid and duplicate registrations", () => {
    ...
    patcher.rpc.register(listContract, { list: () => [] });
    expect(() =>
      patcher.rpc.register(listContract, { list: () => [] }),
    ).toThrow('rpc method "list" is already registered');
    const badContract = defineRpcContract({
      "bad name": { input: z.null(), output: z.array(z.string()) },
    });
    expect(() =>
      patcher.rpc.register(badContract, { "bad name": () => [] }),
...
describe("cli", () => {
  ...
  it("rejects a duplicate registration like the production host", () => {
    ...
    patcher.cli.register(registration);
    expect(() => patcher.cli.register(registration)).toThrow(
...
describe("background", () => {
  it("runService starts once, exposes the AbortController, and resolves on abort", async () => {
    const { patcher, harness } = createFakePluginHost();
    let sawAbort = false;
    patcher.background.service("watcher", {
      start(signal) {
  ...
  it("validates cron expressions at registration and runs schedules on demand", async () => {
    const { patcher, harness } = createFakePluginHost();
    expect(() =>
      patcher.background.schedule("sync", "not-cron", () => {}),
    ).toThrow('invalid cron "not-cron" for schedule "sync"');

    let runs = 0;
    patcher.background.schedule("sync", "*/5 * * * *", () => {
      runs += 1;
    });
...
describe("thread events", () => {
  ...
  it("rejects unknown events at registration", () => {
    const { patcher } = createFakePluginHost();
    expect(() =>
      patcher.events.on("thread.unknown" as "thread.idle", () => {}),
...
describe("sdk", () => {
  ...
  it("keeps nested plugin administration available through the backend SDK", async () => {
    ...
    await expect(patcher.sdk.plugins.catalog.status()).resolves.toEqual(
      catalog,
    );
  ...
  it("throws a stub-naming error for unstubbed methods and accepts late stubs", async () => {
    ...
    expect(() => patcher.sdk.projects.list({})).toThrow(
      "patcher.sdk.projects.list is not stubbed",
    );
    harness.sdk.stub("projects.list", async () => []);
    await expect(patcher.sdk.projects.list({})).resolves.toEqual([]);
...
describe("agent tools", () => {
  ...
  it("rejects duplicate keyed registrations like the production host", () => {
    ...
    patcher.agents.registerTool(tool);
    expect(() => patcher.agents.registerTool(tool)).toThrow(
      'tool "lookup_doc" is already registered',
    );
    patcher.agents.contributeInstructions(() => "first");
    expect(() => patcher.agents.contributeInstructions(() => "second")).toThrow(
  ...
  it("fails closed for unknown and duplicate configure ids", async () => {
    ...
    unknown.patcher.agents.configure(() => ({
      tools: ["missing-tool"],
      skills: ["known-skill"],
    }));
    await expect(
    ...
    duplicate.patcher.agents.configure(() => ({
      tools: [],
...
describe("dispose", () => {
  it("reloads atomically while preserving storage and the direct harness API", async () => {
    ...
    host.patcher.rpc.register(oldContract, { version: () => "old" as const });
    await host.patcher.storage.kv.set("cursor", { page: 2 });
    const oldDatabase = host.patcher.storage.database();
    ...
    await expect(
      host.harness.lifecycle.reload((patcher) => {
        patcher.rpc.register(oldContract, { version: () => "old" as const });
        patcher.rpc.register(oldContract, { version: () => "old" as const });
      }),
    ...
    await expect(host.patcher.storage.kv.get("cursor")).resolves.toEqual({
      page: 2,
    });
    ...
    const replacement = await host.harness.lifecycle.reload(async (patcher) => {
      await expect(patcher.storage.kv.get("cursor")).resolves.toEqual({
        page: 2,
      });
      expect(
        patcher.storage.database().prepare("SELECT value FROM state").get(),
      ).toEqual({ value: "kept" });
      patcher.rpc.register(nextContract, { version: () => "new" as const });
    });

    await expect(replacement.harness.callRpc("version")).resolves.toBe("new");
    await expect(host.patcher.storage.kv.get("cursor")).rejects.toThrow(
  ...
  it("aborts services, runs hooks LIFO, closes the database, and poisons the handle", async () => {
    ...
    patcher.background.service("svc", {
      start(signal) {
        return new Promise<void>((resolve) => {
          signal.addEventListener("abort", () => {
            order.push("aborted");
    ...
    await expect(patcher.storage.kv.get("x")).rejects.toThrow(
      "used a stale API handle",
    );
    expect(() => patcher.sdk).toThrow("stale");
...
describe("realtime and status", () => {
  it("normalizes published payloads and records needs-configuration", () => {
    const { patcher, harness } = createFakePluginHost();
    patcher.realtime.publish("notes-changed", undefined);
    patcher.realtime.publish("notes-changed", { at: new Date(0) });
    ...
    expect(() => patcher.realtime.publish("bad", { boom: 1n })).toThrow(
      "not JSON-serializable",
    );

    patcher.status.needsConfiguration("");
    patcher.status.needsConfiguration("set a token");
