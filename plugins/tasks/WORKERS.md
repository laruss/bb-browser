# Worker guide — patcher-plugin-tasks

You are one of several agents building this plugin in a SHARED worktree on branch `patcher/tasks-plugin`. Read this whole file before writing code.
...
- Visual reference: `tasks-plugin-mock.html` in manager thread storage (Linear-like, Patcher-themed).
...
2. **Git**: commit only your owned paths on `patcher/tasks-plugin` when your checks are green, message prefixed `tasks plugin: `. Never push, never switch branches, never merge.
3. **UI rules** (for any frontend work):
   ...
   - Theme: Patcher tokens only (`bg-background`, `text-muted-foreground`, `border-border`, semantic `--success`/`--attention`/`--timeline-accent`/`--warning`/`--destructive` via their utilities). NO hex/oklch literals, no `text-[Npx]` (AGENTS.md).
...
4. **Tests**: vitest in this package; plugin testing harness for backend (`@patcher/plugin-sdk/testing`, see `server.test.ts`). High-value tests only; never mock sqlite (the harness gives a real one).
5. **Gates before you report done**:
   - `bunx turbo run typecheck --filter=patcher-plugin-tasks`
   - `bunx turbo run test --filter=patcher-plugin-tasks`
   - `bunx turbo run build --filter=patcher-plugin-tasks`
   - UI tasks: visual verification (below) with screenshots in your report.

## Dev instance + reload loop

A dev Patcher instance for THIS worktree is already running with the plugin installed (path install, `provenance: direct`):
